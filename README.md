# wbsgen — 空の WBS を作るツール

期間を指定すると、**日程表と記入用の空行だけの WBS (ガントチャート用紙)** を
Excel で書き出します。中身は入れません。
...
以前はタスクを定義してガントバー・イナズマ線・進捗や遅れの計算まで行う
ツールでしたが、「中身が空のものだけ作る」方針に絞ったため、
日程計算とチャート描画は削除しました。以前の実装はコミット `4735ef1`
にあります。
