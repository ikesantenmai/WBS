...
def main(argv=None) -> int:
    ...
    blank = sub.add_parser(
        "blank",
        help="期間を指定して、中身が空の WBS を作る",
        description="タスクを書かずに、指定した期間の日程表と記入用の空行だけの "
                    "WBS を作ります。出力先の拡張子が .yaml / .json なら定義ファイル、"
                    "それ以外なら Excel を書き出します。",
    )
    blank.add_argument("-o", "--output", default="wbs.xlsx",
                       help="出力先 (既定: wbs.xlsx)")
    blank.add_argument("--start", required=True, help="開始日 (YYYY-MM-DD)")
    period = blank.add_mutually_exclusive_group()
    ...
    blank.add_argument("--unit", choices=("day", "week", "month"), default="week",
                       help="チャート表示単位 (既定: week)")
    blank.add_argument("--rows", type=int, default=None,
                       help=f"記入用の空行数 (既定: {DEFAULT_ROWS})")
    blank.add_argument("--title", default=None, help="プロジェクト名")
    blank.add_argument("--member", action="append", default=None,
                       help="担当者を登録する (複数指定可)")
    blank.set_defaults(func=_cmd_blank)
...
def _cmd_blank(args) -> int:
    ...
    project = build_blank(
        start=start,
        end=_date(args.end, "--end") if args.end else None,
        period_days=args.period_days,
        months=args.months,
        unit=args.unit,
        rows=DEFAULT_ROWS if args.rows is None else args.rows,
        title=args.title or f"{start.year}年 スケジュール",
        members=args.member,
    )
    output = Path(args.output)
    if output.suffix.lower() in (".yaml", ".yml", ".json"):
        _write_definition(project, output)
    else:
        WorkbookRenderer(project).save(output)

    end = start + _dt.timedelta(days=project.chart.period_days - 1)
    print(f"生成しました: {output}")
    print(f"  期間        : {start} 〜 {end} ({project.chart.period_days} 日)")
    print(f"  表示単位    : {project.chart.unit}")
    print(f"  空行        : {project.blank_rows} 行")
    return 0


def _write_definition(project, output: Path) -> None:
    ...
    if output.suffix.lower() == ".json":
        ...
        output.write_text(json.dumps(to_dict(project), ensure_ascii=False, indent=2),
                          encoding="utf-8")
    else:
        output.write_text(to_yaml(project), encoding="utf-8")


def _date(text: str, field: str) -> _dt.date:
    try:
        return _dt.datetime.strptime(text, "%Y-%m-%d").date()
    except ValueError:
        raise ProjectError(f"{field} は YYYY-MM-DD 形式で指定してください: {text!r}")
